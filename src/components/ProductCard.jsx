...
import { Heart, Star, ShoppingCart, Plus, Minus, Eye, Zap, RefreshCw, BarChart2 } from 'lucide-react';
import { useCart } from '../context/CartContext';
import { showToast } from './NotificationToast';
import { Link } from 'react-router-dom';
import QuickViewModal from './QuickViewModal';
import { useCompare } from '../pages/Compare';

const ProductCard = ({ product, compact = false }) => {
    const { addToCart, toggleWishlist, wishlist, cart, updateQuantity, removeFromCart, addLoyaltyPoints, setSidebarOpen } = useCart();
    ...
    return (
        <>
            <motion.div
                ...
                className="group relative bg-white dark:bg-dark-card rounded-3xl overflow-hidden border border-gray-100 dark:border-white/5 shadow-md hover:shadow-2xl hover:shadow-green-500/20 transition-shadow duration-400 magnetic-card card-shine flex flex-col cursor-pointer"
            >
                {/* Top Badges */}
                ...
                <motion.button
                    whileHover={{ scale: 1.15 }}
                    whileTap={{ scale: 0.85 }}
                    onClick={(e) => { e.preventDefault(); addToCompare(product); showToast({ type: 'success', title: 'Added to Compare ⚖️', message: product.title }); }}
                    className={`absolute top-3 left-3 z-20 p-2 rounded-full backdrop-blur-md shadow-md transition-all duration-300 ${isInCompare(product.id)
                        ? 'bg-blue-500 text-white shadow-blue-500/40'
                        : 'bg-white/90 dark:bg-black/50 text-gray-400 hover:text-blue-500'
                        }`}
                    title="Compare"
                >
                ...
                <Link to={`/product/${product.id}`} className="block relative overflow-hidden bg-white dark:bg-gray-800/60" style={{ aspectRatio: '1/1' }}>
                    ...
                    <motion.button
                        ...
                        className="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 flex items-center gap-1.5 bg-black/75 backdrop-blur-sm text-white text-xs font-semibold px-3 py-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 whitespace-nowrap"
                    >
                    ...
                    {product.isSubscribable && (
                        <div className="absolute top-3 left-3 z-20 flex items-center gap-1 px-2 py-0.5 rounded-full bg-teal-500 text-white text-[9px] font-bold shadow-lg">
                            <RefreshCw className="w-2.5 h-2.5" /> Sub & Save 15%
                        </div>
                    )}
                ...
                <div className="flex flex-col flex-1 p-2.5 sm:p-4 gap-1.5 sm:gap-2">
                    ...
                    <div className="flex items-center justify-between pt-2 mt-auto gap-2 min-h-[44px]">
                        ...
                        <motion.div
                            ...
                            className="flex flex-col justify-center px-3 py-1.5 rounded-2xl bg-green-600 hover:bg-green-700 cursor-default select-none transition-colors"
                        ...
                        >
                            <div className="flex items-baseline gap-1">
                                ...
                                {product.unit && (
                                    <span className="text-[8px] text-green-200 leading-none">/{product.unit}</span>
                                )}
                            </div>
                            {discountedPrice && (
                                <span className="text-[8px] text-green-200 line-through leading-none mt-0.5">
